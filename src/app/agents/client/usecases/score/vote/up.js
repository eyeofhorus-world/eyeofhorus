...
export default class ScoreVoteUp {
  ...
  run(userTrackingContext, input, stagingCallback) {
    ...
    .then((score) => {
      if (score.get('upVoted') === true) {
        return helpers.reset(score);
      } else if (score.get('downVoted') === true) {
        return helpers.reset(score).then(resetScore => helpers.toggleUp(resetScore));
      } else {
        return helpers.toggleUp(score);
      }
    })
    ...
    .then(score => self.scoreProvider.voteUp(score.get('_id'), userTrackingContext.toJS()))
    .then((score) => {
      if (score) {
        return Promise.resolve(score);
      }
